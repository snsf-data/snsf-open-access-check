...
if (include_publications) {
  ...
  table_data <- articles %>%
    # When Unpaywall does have the journal name, take it (bc. from Crossref), 
    # otherwise, fallback to Dimensions journal name
    mutate(journal_name = ifelse(!is.na(journal$title), journal$title,
                                 journal_name)) %>% 
    ...
    Journal = map_chr(Journal, function(x) {
      # First, escape any Latex symbols
      # x <- sanitize_tex_string(x)
      x <- knitr:::escape_latex(x)
      
      # Then, create a link to Sherpa/Romeo, search by journal name
      paste0(
        "\\href{https://v2.sherpa.ac.uk/cgi/romeosearch?",
        "publication_title-auto=",
        URLencode(x),
        "&_action_search=Search&screen=Search&publication_title-auto_merge=",
        "ALL}{", x, "}"
      )
    }))
