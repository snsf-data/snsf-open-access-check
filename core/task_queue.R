# Multi Process Task Queue - Credits to Gábor Csárdi
...
task_q <- R6::R6Class(
  ...
  public = list(
    ...
    push = function(fun, args = list(), id = NULL) {
      ...
      private$tasks <- tibble::add_row(private$tasks, .before = before,
                                       id = id, idle = FALSE, state = "waiting",
                                       fun = list(fun),
                                       args = list(args), worker = list(NULL),
                                       result = list(NULL))
      private$schedule()
      invisible(id)
    },
  ...
  private = list(
    ...
    start_workers = function(concurrency) {
      ...
      for (i in seq_len(concurrency)) {
        ...
        private$tasks <- tibble::add_row(private$tasks,
                                         id = paste0(".idle-", i), idle = TRUE,
                                         state = "running",
                                         fun = list(NULL), args = list(NULL),
                                         worker = list(rs),
                                         result = list(NULL))
      }
    },
    ...
    schedule = function() {
      ...
      waiting <- which(private$tasks$state == "waiting")[seq_len(length(ready))]
